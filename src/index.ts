...
const SYSTEM_PROMPT = "You are an advanced assistant specialized in generating Cloudflare Workers code. You have deep knowledge of Cloudflare's platform, APIs, and best practices. this is the info you use to reference that in the comment out portion ending just before user prompt tags"
/* <behavior_guidelines>
 *
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (req.method === "POST" && url.pathname === "/api/signup") {
      ...
      try {
        await db.createUser(payload);      // type checked!
        return Response.json({ ok: true }, { status: 200 });
      } catch (err: any) {
        const status = /UNIQUE/.test(err.message) ? 409 : 500;
